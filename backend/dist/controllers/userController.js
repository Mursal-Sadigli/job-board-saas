...
exports.getNotificationSettings = exports.updateNotificationSettings = exports.getUserProfile = exports.updateUserProfile = exports.deleteUserResume = exports.getUserResumes = exports.uploadUserResume = exports.syncUser = void 0;
...
const syncUser = async (req, res) => {
    try {
        ...
        let role = req.auth?.sessionClaims?.metadata?.role?.toUpperCase() || client_1.UserRole.CANDIDATE;
        ...
        if (!email) {
            ...
            try {
                ...
                role = metaRole?.toUpperCase() || client_1.UserRole.CANDIDATE;
        ...
        let userByClerk = await prisma_1.default.user.findUnique({
            where: { clerkId },
            include: { resumes: true }
        });
        // 2. Try to find user by email
        let userByEmail = await prisma_1.default.user.findUnique({
            where: { email },
            include: { resumes: true }
        });
        let finalUser;
        if (userByClerk && userByEmail && userByClerk.id !== userByEmail.id) {
            ...
            // Cleanup
            await prisma_1.default.user.delete({ where: { id: userByClerk.id } }).catch(e => console.error('Cleanup error:', e));
            ...
            // Create new user
            finalUser = await prisma_1.default.user.create({
                data: {
                    clerkId,
                    email,
                    name,
...
const uploadUserResume = async (req, res) => {
    try {
        const clerkId = req.auth?.sessionClaims?.sub;
        if (!clerkId) {
        ...
        // Save to database
        const newResume = await prisma_1.default.resume.create({
            data: {
                userId: user.id,
...
const deleteUserResume = async (req, res) => {
    try {
        ...
        await cloudinary_1.v2.uploader.destroy('user-resumes/' + publicId, { resource_type: 'raw' });
...
exports.deleteUserResume = deleteUserResume;
const updateUserProfile = async (req, res) => {
    try {
        ...
        const { firstName, lastName, title, bio, location, phone, skills, experience, education, socialLinks } = req.body;
